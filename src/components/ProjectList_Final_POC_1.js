...
const ProjectList_Final = () => {
 ...
 /*   const fetchData = useCallback(() => getProjectData(queryPageIndex,queryPageSize), [projects]);
...
*/
    /*useEffect(() => {
        //getProjectData(queryPageIndex,queryPageSize);
        fetch(`http://localhost:8080/api/v1/project/find-all?page=${page}&size=${pageSize}`)
    ...
    const showDeleteModal = (entityType, id) => {
        ...
        //setDeleteMessage(`Are you sure you want to delete the ${entityType} '${projects && projects.find((x) => x.projectId === id)?.projectName}' with ID: ${id}?`);
    ...
    //This piece of code written to get the map projects to state variable, so that in Delete confirmation message we can show the Project name
    //Reference: https://www.geeksforgeeks.org/how-to-fetch-data-from-apis-using-asynchronous-await-in-reactjs/
    useEffect(() => {
        /*const loadPost = async (page, pageSize) => {
      ...
      /*  const data=getProjectData(queryPageIndex,queryPageSize);
        setProjects(data.content)
  ...
  */
        ...
        // Call the function
        //loadPost(queryPageIndex,queryPageSize);
    }, [dataRefresh,loc.state,entityType]);
